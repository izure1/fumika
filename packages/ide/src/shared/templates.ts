...
export const BLUEPRINT_RUNTIME_CODE = [
  ...
  "      const obj = evaluatePin(nodeId + '__object')",
  ...
  "      nextPinId = cond ? 'true' : 'false'",
    ...
    "      const slot = Number(evaluatePin(currentNodeId + '__slot') ?? 0)",
    "      if (tabName !== 'command') {",
  ...
  "      const ms = Number(evaluatePin(currentNodeId + '__ms') ?? 0)",
  "      const callbackFn = evaluatePin(currentNodeId + '__callback')",
  "      const respectSkip = evaluatePin(currentNodeId + '__respectSkip') ?? false",
  "      let timerId = null",
  ...
  "      const timerId = evaluatePin(currentNodeId + '__timerId')",
  ...
  "        while (evaluatePin(currentNodeId + '__condition')) {",
  "          callbackFn()",
  "        }",
  "      }",
  "    }",,
